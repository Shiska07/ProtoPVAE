...
from .receptive_field import compute_rf_prototype, find_high_activation_crop
...
def find_k_nearest_patches_to_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
                                         model_parallel, # pytorch network with prototype_vectors
                                         k=5,
    ...
    start = time.time()
    n_prototypes = model_parallel.module.num_prototypes
    
    prototype_shape = model_parallel.module.prototype_shape
    max_dist = prototype_shape[1] * prototype_shape[2] * prototype_shape[3]

    protoL_rf_info = model_parallel.module.proto_layer_rf_info
    ...
    for idx, (search_batch_input, search_y) in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            # using mean distnces for global analysis as we are trying to fing the
            # closest images to prototypes
            protoL_input_torch, proto_dist_torch = \
                model_parallel.module.push_forward(search_batch)
        ...
        for img_idx, distance_map in enumerate(proto_dist_):
            for j in range(n_prototypes):
                ...
                if full_save:
                    ...
                    if model_parallel.module.prototype_activation_function == 'log':
                        act_pattern = np.log((distance_map[j] + 1)/(distance_map[j] + model_parallel.module.epsilon))
                    elif model_parallel.module.prototype_activation_function == 'linear':
    ...
    for j in range(n_prototypes):
        ...
        if full_save:
            ...
            dir_for_saving_images = os.path.join(root_dir_for_saving_images,
                                                 str(j))
            create_dir(dir_for_saving_images)
