...
from .eval_datasets import Cub2011Eval
from .preprocess import mean, std
from .local_parts import id_to_path, id_to_part_loc, id_to_bbox, part_num, in_bbox
...
@torch.no_grad()
def get_corresponding_object_parts(ppvae_model, args, half_size, use_noise=False):
    ppvae_model.eval()
    ppvae_model_without_ddp = ppvae_model.module if hasattr(ppvae_model, 'module') else ppvae_model
    img_size = ppvae_model_without_ddp.img_size
    proto_per_class = ppvae_model_without_ddp.prototype_block.num_prototypes_per_class
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset,
                                              batch_size=args.test_batch_size, num_workers=4, pin_memory=True, drop_last=False, shuffle=False)
    ...
    for _, (data, targets, img_ids) in tqdm(enumerate(test_loader)):
        ...
        # using mean distances
        _, distances = ppvae_model_without_ddp.push_forward(data,
                                                            use_samp=bool(args.use_sample))
        proto_acts = ppvae_model_without_ddp.prototype_block.distance_2_similarity(
            distances)
...
def evaluate_consistency(ppvae_model, args, half_size=36, part_thresh=0.8):
    all_proto_to_part, all_proto_part_mask = get_corresponding_object_parts(ppvae_model, args, half_size)
...
def evaluate_stability(ppvae_model, args, half_size=36):
    all_proto_to_part, _ = get_corresponding_object_parts(ppvae_model, args, half_size, use_noise=False)
    all_proto_to_part_noise, _ = get_corresponding_object_parts(ppvae_model, args, half_size, use_noise=True)
